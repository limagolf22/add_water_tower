...
char filename_w[]="water_tower.xml";
...
int main() {
    ...
    while (fgets(tampon,200,file_r)!=NULL) {
        ...
        if (is_deg) {
            ...
            if (check_confo) {
                printf("problème de synthaxe, verifier le fichier .csv\n");
                return 1;
            }
...
float get_lon(char coord[]) {
    int a,b,c;
    char d;
    //sscanf(coord,"%i%[^0-9] %i\' %i\" %c",&a,&b,&c,&d);
    sscanf(coord,"%i%*[^0-9]%i%*[^0-9]%i%*[^A-Z]%c",&a,&b,&c,&d);
    printf("%i %i %i %c\n",a,b,c,d);
    if (d=='E') {
        return (a+b/60+c/3600);
